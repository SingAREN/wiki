...
1. Setup the `sync.sh` and `console.sh` scripts in `/opt/saml/scripts`. Replace `[SAML_APP_PASSWORD]` with contents of `/opt/saml/keypairs/passwords/saml_app_password` and [KEY] with contents of `/opt/saml/keypairs/passwords/saml_secret_key_base`/
